...
public interface CategoryFKRepository extends JpaRepository<CategoryFK, String>  {
    @Query("SELECT n FROM CategoryFK n WHERE (:companyId LIKE '' OR n.companyId.id LIKE %:companyId%) AND (:cate1Id LIKE '' OR n.cate1Id.id LIKE %:cate1Id%) AND (:cate2Id LIKE '' OR n.cate2Id.id LIKE %:cate2Id%)")
    ...
    @Query("SELECT n FROM CategoryFK n WHERE n.companyId.id LIKE :companyId AND n.cate1Id.id LIKE :cate1Id AND n.cate2Id.id IS NOT NULL")
    List<CategoryFK> getCategoryFKCate1HadCate2(@Param("companyId") String companyId, @Param("cate1Id") String cate1Id);
}
